...
<script>
    ...
    export default {
        ...
        async asyncData({ params, $http }) {
            ...
            const imgs = await $http.$get(`http://carlos.local/wp-json/filebird/public/v1/attachment-id/?folder_id=1`);
            const n = getRandomInt(imgs.data.attachment_ids.length - 1);
            const id = await imgs.data.attachment_ids[n];
            const img = await $http.$get(`http://carlos.local/wp-json/wp/v2/media/${id}`);
            return { img, imgs, id }
        },
        fetchOnServer: true,
    }
